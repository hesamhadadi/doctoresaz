...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (patch.title || patch.slug) patch.slug = await uniqueSlug(Book, patch.slug || patch.title, params.id);
  const doc = await Book.findByIdAndUpdate(params.id, patch, { new: true });
