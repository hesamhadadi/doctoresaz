...
export async function POST(req: NextRequest) {
  ...
  // آیا این شماره قبلاً ثبت‌نام کرده؟ (تا فرانت فقط از کاربر جدید نام بخواهد)
  const existing = await User.findOne({ phone }).select('_id').lean<any>();
  return ok({ ok: true, message: 'کد تأیید پیامک شد', isNewUser: !existing });
}
