...
export async function GET(req: NextRequest) {
  ...
  const { category, q, min, max, sort = 'newest', inStock, featured, instrument, brand } = Object.fromEntries(sp);
  ...
  if (brand) filter.brand = { $in: String(brand).split(',').filter(Boolean) };
  ...
  // لیست برندهای موجود در همین فیلتر (برای نمایش در سایدبار فروشگاه)
  const brandFilter = { ...filter };
  delete (brandFilter as any).brand;
  ...
  const brands = brandAgg.map((b: any) => ({ name: b._id, count: b.n }));

  return ok({ items, total, page, pages: Math.ceil(total / limit), brands });
}
