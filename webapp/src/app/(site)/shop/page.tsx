...
function ShopInner() {
  ...
  const activeCount = [q, category, params.get('min'), params.get('max'), params.get('brand'), inStock ? '1' : ''].filter(Boolean).length;
  ...
  const Filters = () => (
    <div className="space-y-7">
      ...
      {data.brands?.length > 0 && (
        <div>
          <h4 className="mb-3 flex items-center gap-2 text-sm font-semibold text-ink-100"><Icon name="award" size={15} className="text-firooze-500" /> برند</h4>
          <ul className="max-h-56 space-y-1 overflow-y-auto pl-1">
            {data.brands.map((b: any) => {
              const on = selectedBrands.includes(b.name);
              return (
                <li key={b.name}>
                  <button onClick={() => toggleBrand(b.name)} className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-right text-[12px] transition-colors ${on ? 'bg-firooze-500/12 font-medium text-firooze-600' : 'text-ink-300 hover:bg-ink-800'}`}>
                    <span className="flex items-center gap-2">
                      <span className={`grid h-4 w-4 place-items-center rounded border transition-colors ${on ? 'border-firooze-500 bg-firooze-500 text-white' : 'border-ink-600'}`}>{on && <Icon name="check" size={11} />}</span>
                      {b.name}
                    </span>
                    <span className="num text-[10px] text-ink-500">{num(b.count)}</span>
                  </button>
                </li>
              );
            })}
      ...
      <div className="flex gap-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          {loading ? <SkeletonGrid count={9} /> : !data.items.length ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">{data.items.map((p: any) => <ProductCard key={p._id} product={p} />)}</div>
